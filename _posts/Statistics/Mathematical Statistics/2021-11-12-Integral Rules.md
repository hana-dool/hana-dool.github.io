...
> $$
> In Leibniz notation, if $y=f(u)$ and $u=g(x)$ are both differentiable functions, then $\frac{d y}{d x}=\frac{d y}{d u} \frac{d u}{d x}$
...
$$
\begin{aligned}
\frac{d}{d x}\left(x^{2}+y^{2}\right) &=\frac{d}{d x}(25) \\
